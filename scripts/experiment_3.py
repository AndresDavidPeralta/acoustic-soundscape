...
experiment_dir = r".../Segmented_data/birdclef-2025/experimento_3"
...
with open(log_path, "w", encoding="utf-8") as log_file:
    ...
    for subdir_name in os.listdir(source_dir):
        ...
        for file_name in os.listdir(subdir_path):
            ...
            # Save higher energy segment on train
            start_train = max_energy_idx * samples_per_segment
            end_train = start_train + samples_per_segment
            segment_train = y[start_train:end_train]

            train_name = f"{subdir_name}_{base_stem}_{max_energy_idx}{ext}"
            train_save_path = os.path.join(train_dir, train_name)

            sf.write(train_save_path, segment_train, sr)
            log_file.write(f"{train_name}  train\n")

            # Save the rest of the segments in test
            for i in range(num_segments):
                if i == max_energy_idx:
                    continue  # avoid duplicate segments
                ...
                test_name = f"{subdir_name}_{base_stem}_{i}{ext}"
                test_save_path = os.path.join(test_dir, test_name)
                sf.write(test_save_path, segment, sr)
                log_file.write(f"{test_name} → test\n")

print("==== [INFO] Processing completed. Segments saved successfully and log generated ====")
